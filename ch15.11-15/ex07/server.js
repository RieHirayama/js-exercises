...
function cspMiddleware(_url, req, res) {
  // CSP ヘッダを設定する
  // script-src 'unsafe-inline': インラインスクリプト許可（"RICOH" を表示）
  // script-src 'self': 同一オリジン全部のスクリプト許可なので記載しない（許可しない）
  // script-src hello.jsの“フルURL（少なくともオリジン込み）”:  hello.js を許可（"Hello" を表示）
  // world.js は許可しないため、"World" は表示されない
