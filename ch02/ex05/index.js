...
class DefaultMap extends Map {
    constructor(defaultValue) {
        super()    // 親クラスMapのコンストラクタを呼び出す
        this.defaultValue = defaultValue // デフォルト値を記憶する
    }

    get(key) {
        if (this.has(key)) {        // キーがマップ上に存在する場合
            return super.get(key)  // 親クラス中の値を返す
        } else {
            return this.defaultValue  // キーがマップ上に存在しない場合は、デフォルト値を返す
        }
    }
}
...
class Histogram {
    ...
    add(text) {
        ...
        for (let character of text) {
            ...
            this.letterCounts.set(character, count+1) // １増やす
            this.totalLetters++
        }
    }
    ...
    toString() {
        ...
        entries.sort((a,b) => {      // ソート順を定義する関数
            if (a[1] === b[1]) {
                return a[0] < b[0] ? -1 : 1 // 文字数が同じ場合は、アルファベット順でソートする。
            } else {
                return b[1] - a[1] // 文字数が異なる場合は、降順でソートする。
            }
        })
...
// この async関数（Promiseを返す関数）は、Histogramオブジェクトを生成する。
...
// 標準入力からHistogramオブジェクトを生成し、非ストグラムを表示する。
